...
def merge_bin(sub, i):
    ...
    upper = last['bin_up']
    # 空DF初始化（指定列名）
    df = pd.DataFrame(columns=['bin', 'bin_low', 'bin_up', 'total'])
    # 构造单行DF（与空DF列名对齐）
    new_row = pd.DataFrame([[i, lower, upper, total]], columns=df.columns)
    # 用pd.concat替代append
...
def cont_var_bin(x, y, method, mmin=5, mmax=10, bin_rate=0.01, stop_limit=0.1, bin_min_num=20):
    ...
    if mmin >= df_temp_all.shape[0]:
        ...
        for i in temp_Map1['bin'].unique():
            ...
            data = pd.concat([data, rowdata], ignore_index=True)
...
#    y = data_train.target
    ...
    if np.issubdtype(x.dtype, np.float64):
        x = x.astype('str')
...
def disc_var_bin_map(x, bin_map):
    """
    用离散变量分箱后的结果，对原始值进行分箱映射（修复维度不匹配+布尔型兼容）
    ##参数
    x: 待分箱映射的离散变量，pandas Series
    bin_map:分箱映射字典， pandas dataframe（需包含var_name、bin列）
    ##返回值
    返回映射结果（pandas Series）
    """
    # 步骤1：深拷贝避免修改原数据，单独处理缺失值
    x_copy = x.copy(deep=True)
    na_mask = pd.isnull(x_copy)  # 缺失值掩码（布尔数组）
    xx = x_copy[~na_mask]  # 非缺失值部分

    # 步骤2：全类型统一转换为字符串（新增布尔型兼容）
    if xx.dtype == 'bool':
        # 布尔型特殊处理：True→'True'，False→'False'
        xx = xx.astype('str')
    elif np.issubdtype(xx.dtype, np.int_):
        xx = xx.astype('float').astype('str')
    elif np.issubdtype(xx.dtype, np.float64):
        xx = xx.astype('str')

    # 步骤3：构建映射字典（兼容var_name用%分隔的多值场景）
    d = {}
    for i in bin_map.index:
        var_names = bin_map.loc[i, 'var_name'].split('%')
        target_bin = bin_map.loc[i, 'bin']
        for j in var_names:
            if j.strip() != 'NA':  # 过滤空字符串
                d[j.strip()] = target_bin  # 去除首尾空格，避免匹配失败

    # 步骤4：非缺失值映射（避免直接修改原Series导致的索引问题）
    xx_mapped = xx.map(d)
    new_x = pd.Series(index=x_copy.index, dtype='float64')
    new_x[~na_mask] = xx_mapped  # 非缺失值赋值

    # 步骤5：缺失值映射（核心修复：维度匹配+安全赋值）
    na_count = na_mask.sum()
    if na_count > 0:
        # 找到NA对应的分箱值
        na_bin_mask = bin_map['var_name'].str.strip() == 'NA'
        if na_bin_mask.any():
            na_bin_value = bin_map.loc[na_bin_mask, 'bin'].iloc[0]  # 取第一个匹配的bin值
            # 赋值：确保值的长度与缺失值数量一致
            new_x[na_mask] = na_bin_value
        else:
            # 无NA分箱时，缺失值映射为NaN
            new_x[na_mask] = np.nan
...
if __name__ == '__main__':
    ...
    path = 'C:\\Users\\98128\\Desktop\\量化相关\\职业规划\\credit_risk_portfolio\\demo\\variable_bin_methods'
