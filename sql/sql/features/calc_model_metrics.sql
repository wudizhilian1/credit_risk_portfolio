...
WITH score_bad AS (
    ...
    WHERE dt BETWEEN '2024-01-01' AND '{{eval_date}}'   -- 可根据需要调整范围
...
,score_today AS (
    SELECT
        ...
        COUNT(*) AS cnt
    FROM feature_apply_broad
    WHERE dt = '{{eval_date}}' AND pred_score IS NOT NULL
    GROUP BY score_bin
),
score_base AS (
    SELECT
        ...
        COUNT(*) AS cnt
    FROM feature_apply_broad
    WHERE dt = '{{base_date}}' AND pred_score IS NOT NULL
    GROUP BY score_bin
),
